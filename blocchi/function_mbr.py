...
def run_blustClust(cartella, identita, name, identity):
    ...
    os.mkdir('not_similar'+name+'_'+identity+'/')
    os.chdir(cartella)
    os.system('for file in *; do blastclust -i $file -o ../not_similar'+name+'_'+identity+'/$file -p F -S '+identita+' ; done')
    os.chdir('../')
...
def filtro_n_seq(cartella, n_seq_family, name, identity):  
    ...
    os.mkdir('filtro_n_seq'+name+'_'+identity+'/')
    ...
    for fam in lista_fam:
        ...
        if int(output.strip().split()[0])>=int(n_seq_family):
            os.system('cp '+cartella+'/'+fam+' filtro_n_seq'+name+'_'+identity+'/'+fam)
...
def get_bear(cartella, cartella_bear, name, identity):    
    ...
    os.mkdir('bear_filtered'+name+'_'+identity+'/')
    ...
    for fam_clean in lista_fam_filter:
        ...
        o=open('bear_filtered'+name+'_'+identity+'/'+fam_clean, "w")
...
def add_gap(cartella, seed_rfam, name, identity):
    ...
    os.mkdir('bear_alignment'+name+'_'+identity+'/')
    c=0
    for famiglia in lista_fam_filter2:
        c+=1
        o=open('bear_alignment'+name+'_'+identity+'/'+famiglia, "w")
...
def convert_new_bear(cartella, name, identity):
    ...
    os.mkdir('bear_new_alignment'+name+'_'+identity+'/')
    
    
    for famiglia in famiglie_bear:
        o=open('bear_new_alignment'+name+'_'+identity+'/'+famiglia, "w")
        f=open(cartella+famiglia)
        line=f.readline()
        while(line):
            o.write(decode(line)+"\n")
...
def crea_blocchi(cartella, name, identity):
    ...
    os.mkdir('blocchi_new_bear'+name+'_'+identity+'/')
    famiglie=os.listdir(cartella)
    for fam in famiglie:
        o=open('blocchi_new_bear'+name+'_'+identity+'/'+fam, "w")
...
def sostituzioni_osservate(cartella, v_bear, name, identity):
    ...
    sostituzioni.to_csv('sostituzioni'+name+'_'+identity+'.tsv', sep="\t") 
    return sostituzioni
...
def make_q(sostituzioni, v_bear, name, identity):
    ...
    q_ij.to_csv('q_ij'+name+'_'+identity+'.tsv', sep="\t")
...
def make_e(p_i, v_bear, name, identity):
    ...
    e_ij.to_csv('E_ij'+name+'_'+identity+'.tsv', sep='\t')
    return e_ij
...
def crea_matrice(freq_observed, fr_attese, name, identity):
    ...
    probability_matrix.to_csv('matrice_probabilita'+name+'_'+identity+'.tsv', sep="\t")
...
def CheVojDePiu(folder, folder_bear, RFAM_seed_file, id_blustClust, filtro_nSeq, file_alph):
    ...
    filtro_n_seq('not_similar'+name+'_'+identity+'/', filtro_nSeq, name, id_blustClust)
    get_bear('filtro_n_seq'+name+'_'+identity+'/', folder_bear, name, id_blustClust)
    add_gap('bear_filtered'+name+'_'+identity+'/', RFAM_seed_file, name, id_blustClust)
    convert_new_bear_file('bear_alignment'+name+'_'+identity+'/', file_alph, name, id_blustClust)

    crea_blocchi('bear_new_alignment'+name+'_'+identity+'/', name, id_blustClust)
    
    v_char=[x.split()[1] for x in f]
    sost=sostituzioni_osservate('blocchi_new_bear'+name+'_'+identity+'/', v_char, name, id_blustClust)
